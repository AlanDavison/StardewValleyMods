...
namespace BetterReturnScepter
{
    public class ModEntry : Mod
    {
        ...
        private WandPatches patches;
        private ModConfig config;

        private PreviousPoint previousPoint = new PreviousPoint();
        private RodCooldown rodCooldown;
        ...
        private void ButtonsChanged(object? sender, ButtonsChangedEventArgs e)
        {
            ...
            if (rodCooldown.CanWarp)
            {
                ...
                {
                    // And if right click or the controller bind is pressed...
                    if (e.Pressed.Contains(SButton.MouseRight) || config.ReturnToLastPoint.JustPressed())
                    {
                        // And if the conditions are correct to allow a wand warp...
                        if (!player.bathingClothes && player.IsLocalPlayer && !player.onBridge)
                        {
                            ...
                            for (int i = 0; i < 12; i++)
                            {
                                TemporaryAnimatedSprite zoomyDust = new TemporaryAnimatedSprite(354, Game1.random.Next(25, 75), 6, 1, new Vector2(Game1.random.Next((int)player.position.X - 256, (int)player.position.X + 192), Game1.random.Next((int)player.position.Y - 256, (int)player.position.Y + 192)), flicker: false, (Game1.random.NextDouble() < 0.5) ? true : false);
