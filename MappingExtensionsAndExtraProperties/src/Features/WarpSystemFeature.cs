...
public class WarpSystemFeature : Feature
{
    ...
    public WarpSystemFeature(Harmony harmony, string id, Logger logger, TilePropertyHandler tilePropertyHandler, Properties propertyUtils)
    {
        this.Enabled = false;
        this.HarmonyPatcher = harmony;
        this.FeatureId = id;
        WarpSystemFeature.logger = logger;
        WarpSystemFeature.tileProperties = tilePropertyHandler;
        WarpSystemFeature.propertyUtils = propertyUtils;
        this.AffectsCursorIcon = true;
        this.CursorId = 5;

        GameLocation.RegisterTileAction("MEEP_WarpStation", this.DoWarpStationInteraction);
    }
    ...
    private bool DoWarpStationInteraction(GameLocation location, string[] propertyArgs, Farmer player, Point tile)
    {
        if (!enabled)
            return false;
